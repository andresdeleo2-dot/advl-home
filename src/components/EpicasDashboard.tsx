...
// Paleta para el color auto-asignado de un Feature nuevo (misma paleta que las épicas semilla).
...
export default function EpicasDashboard({ initialEpics }: { initialEpics: Epica[] }) {
  ...
  useEffect(() => {
    ...
    const applyFromUrl = () => {
      ...
      // et/os: pestaña Objetivos (pantalla completa) + en qué nodo estaba parado. Se leen
      // juntos porque os sin et==='obj' no significa nada (la pestaña Tareas no usa objSel).
      const et = q.get('et')
      if (et === 'obj') {
        setEpicTab('objetivos')
        const os = q.get('os')
        const parts = os ? os.split(':') : []
        if (parts[0] === 'f' && parts[1]) setObjSel({ kind: 'feature', fId: parts[1] })
        else if (parts[0] === 'i' && parts[1] && parts[2]) setObjSel({ kind: 'iniciativa', fId: parts[1], iniId: parts[2] })
  ...
  useEffect(() => {
    ...
    if (epicTab === 'objetivos') {
      q.set('et', 'obj')
      const os = objSel.kind === 'feature' ? `f:${objSel.fId}`
        : objSel.kind === 'iniciativa' ? `i:${objSel.fId}:${objSel.iniId}`
        : objSel.kind === 'sinIniciativa' ? `si:${objSel.fId}`
        : objSel.kind === 'sinFeature' ? 'sf'
        : ''
      if (os) q.set('os', os)
    }
    const next = `${window.location.pathname}?${q.toString()}`
    if (next !== window.location.pathname + window.location.search) window.history.replaceState(null, '', next)
  }, [planMode, viewDate, featuredId, planFilter, weekEpica, weekDif, epicTab, objSel])
  ...
  // Calificación del día (1-10, con decimales). Se guarda junto al comentario (mismo día).
  useEffect(() => { try { const raw = localStorage.getItem('epicas.dayScores.v1'); if (raw) setDayScores(JSON.parse(raw)) } catch { /* noop */ } }, [])
  const setDayScore = (day: string, score: number | null) => setDayScores(prev => {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'k' && !typing) {
        ...
        // el fondo o "Cancelar" — para que si hay cambios sin guardar se cacheen como borrador
        // ("Borrador restaurado ↩" la próxima vez). Antes hacía setTaskEdit(null) directo: Escape,
    ...
    // taskDraft/taskEditTarget entran aquí SÓLO por closeTaskEdit(): sin ellos, el handler de Escape
    // quedaría cerrado sobre el borrador de cuando se abrió el editor (taskEdit no cambia mientras
  ...
  /** Revierte UNA épica a su snapshot de antes del cambio optimista (capturado por quien llama,
  ...
  const setTaskFeature = (e: Epica, ti: number, featureId: string | null) => {
    const tasks = clone(e.tasks)
    const t = tasks[ti]; if (!t) return
    if (featureId) { if (featuresReady.current) t.featureId = featureId }
    ...
    const titulo = full
      ? (
        <input defaultValue={k.t} key={`obj:${k.id}:t`} aria-label="Nombre del objetivo"
          onKeyDown={ev => { if (ev.key === 'Enter') (ev.target as HTMLInputElement).blur(); if (ev.key === 'Escape') { ev.stopPropagation(); (ev.target as HTMLInputElement).value = k.t; (ev.target as HTMLInputElement).blur() } }}
      ...
      <div key={t.id ?? t._i} className="ep-obj-task" style={{
        ...
        <div style={{ minWidth: 0, display: 'flex', flexDirection: 'column', gap: 3 }}>
          <input defaultValue={t.t} key={`task:${t.id}:t`} disabled={bloqueada} aria-label="Título de la tarea"
            onKeyDown={ev => { if (ev.key === 'Enter') (ev.target as HTMLInputElement).blur(); if (ev.key === 'Escape') { ev.stopPropagation(); (ev.target as HTMLInputElement).value = t.t; (ev.target as HTMLInputElement).blur() } }}
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 8px 2px' }}>
        ...
        <input value={activo ? objQuickTaskName : ''} onFocus={() => { setObjQuickTask(key); setObjQuickTaskName('') }}
          ...
          onKeyDown={ev => {
            if (ev.key === 'Escape') { ev.stopPropagation(); setObjQuickTask(null); setObjQuickTaskName('') }
    ...
    active ? (
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
        <input autoFocus value={name} onChange={ev => setName(ev.target.value)}
          onKeyDown={ev => { if (ev.key === 'Enter') onCommit(); if (ev.key === 'Escape') { ev.stopPropagation(); onCancel() } }}
    ...
    return (
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
        ...
        <input autoFocus value={quickObjName} onChange={ev => setQuickObjName(ev.target.value)}
          onKeyDown={ev => { if (ev.key === 'Enter') crear(); if (ev.key === 'Escape') { ev.stopPropagation(); setQuickObjOwner(null); setQuickObjName('') } }}
        ...
        {featQuickAdd ? (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
            <input autoFocus value={featQuickName} onChange={ev => setFeatQuickName(ev.target.value)}
              ...
              onKeyDown={ev => { if (ev.key === 'Enter') commitQuickFeature(epicaId, onPick); if (ev.key === 'Escape') { ev.stopPropagation(); setFeatQuickAdd(false); setFeatQuickName('') } }}
  ...
  const passEpicFeature = (t: (typeof indexed)[number]) => {
    if (epicFeatureFilter === 'todas') return true
      ...
      <>
          ...
          <div className="ep-modal-body ep-editor-body" style={{ padding: '10px 28px 22px', maxHeight: inline ? 'none' : '72vh', overflow: inline ? 'visible' : 'auto' }}>
            ...
            <div style={cardEd}>
              ...
              {renderKpisEditor(d.kpis, next => patchDraft(x => ({ ...x, kpis: next })))}
            </div>
            ...
            {/* Features (Épica → Feature → Tarea): agrupan tareas grandes; cada uno puede tener sus propios objetivos */}
            <div style={cardEd}>
              <div style={secHead}>
                <div><label style={{ ...lbl, marginTop: 0 }}>Features</label><div style={{ fontSize: 11, color: 'rgba(20,35,61,0.5)', marginTop: 3 }}>Agrupa tareas grandes dentro de esta épica. Cada uno puede tener sus propios objetivos.</div></div>
                <button onClick={() => {
                  const next: EpicaFeature = { id: uid(), t: '', color: FEATURE_COLORS[(d.features || []).length % FEATURE_COLORS.length] }
                  patchDraft(x => ({ ...x, features: [...(x.features || []), next] }))
                  setFeatOpenId(next.id)
                }} style={addBtn}>+ Feature</button>
              </div>
              {!featuresReady.current && <div style={{ fontSize: 10.5, color: 'rgba(176,82,46,0.9)', marginTop: 6 }}>Corre sql/epicas-12-features.sql en Supabase para guardar Features.</div>}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 10 }}>
                {(d.features || []).length === 0 && <div style={{ fontSize: 11.5, color: 'rgba(20,35,61,0.5)' }}>Sin Features aún.</div>}
                {(d.features || []).map((f, fi) => {
                  const open = featOpenId === f.id
                  const taskN = d.tasks.filter(t => t.featureId === f.id).length
                  const kpiN = (f.kpis || []).length
                  const setFeat = (patch: Partial<EpicaFeature>) => patchDraft(x => ({ ...x, features: (x.features || []).map((y, j) => (j === fi ? { ...y, ...patch } : y)) }))
                  return (
                    <div key={f.id} style={{ border: '1px solid rgba(15,35,64,0.10)', borderRadius: 11, background: '#fff', overflow: 'hidden' }}>
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '9px 10px' }}>
                        <span style={{ flexShrink: 0, height: 10, width: 10, borderRadius: 99, background: f.color || '#5B6B86' }} />
                        <input value={f.t} onChange={e => setFeat({ t: e.target.value })} placeholder="Nombre del feature" style={inpSmall} />
                        <span style={{ flexShrink: 0, fontSize: 10.5, fontWeight: 700, color: 'rgba(20,35,61,0.5)', whiteSpace: 'nowrap' }}>{taskN} {taskN === 1 ? 'tarea' : 'tareas'} · {kpiN} KPIs</span>
                        <button onClick={() => setFeatOpenId(open ? null : f.id)} aria-expanded={open} title={open ? 'Ocultar objetivos' : 'Ver/editar objetivos'} style={{ flexShrink: 0, cursor: 'pointer', border: '1px solid rgba(15,35,64,0.14)', background: '#fff', borderRadius: 8, height: 28, width: 28, color: 'rgba(20,35,61,0.5)' }}>
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6" style={{ transform: open ? 'rotate(180deg)' : 'none', transition: 'transform .15s' }}><path d="m6 9 6 6 6-6" /></svg>
                        </button>
                        <button aria-label="Eliminar feature" title="Las tareas que tenía quedan sin Feature (no se borran)" onClick={() => {
                          patchDraft(x => ({
                            ...x,
                            features: (x.features || []).filter((_, j) => j !== fi),
                            tasks: x.tasks.map(t => (t.featureId === f.id ? { ...t, featureId: '' } : t)),
                          }))
                          if (open) setFeatOpenId(null)
                        }} style={delBtn}>✕</button>
                      </div>
                      {open && (
                        <div style={{ padding: '2px 10px 12px', borderTop: '1px solid rgba(15,35,64,0.08)' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 10 }}>
                            <span style={{ font: '700 10px/1 var(--font-ui)', letterSpacing: '.1em', textTransform: 'uppercase', color: 'rgba(20,35,61,0.5)' }}>Objetivos del feature</span>
                            <button onClick={() => setFeat({ kpis: [...(f.kpis || []), { id: uid(), t: '' }] })} style={addBtn}>+ Objetivo</button>
                          </div>
                          {(f.kpis || []).length === 0
            ...
            <div role="dialog" aria-modal="true" aria-label="Detalle de la tarea" onClick={e => e.stopPropagation()} className="ep-modal" style={opts.docked ? { width: '100%', maxWidth: 'none', height: '100%', background: '#fff', borderRadius: 14, border: '1px solid rgba(15,35,64,0.10)', overflow: 'hidden', display: 'flex', flexDirection: 'column' } : { width: '100%', maxWidth: 920, background: '#fff', borderRadius: 18, boxShadow: '0 40px 80px -30px rgba(8,18,36,.7)', overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: 'calc(100dvh - 32px)' }}>
              ...
              <div className="ep-modal-head" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, padding: '18px 26px 14px', borderBottom: '1px solid rgba(15,35,64,0.08)', flexShrink: 0 }}>
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ marginBottom: 16 }}>
                      <div style={eb}>Contribuye a (del feature)</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                        <select value={actualF?.id || ''} onChange={ev => setTaskFeatureMilestone(ep, feat.id, t.id!, ev.target.value || null)}
                          style={{ cursor: 'pointer', border: '1px solid rgba(15,35,64,0.14)', borderRadius: 9, padding: '7px 9px', fontSize: 12.5, fontWeight: 600, color: actualF ? '#16365F' : 'rgba(20,35,61,0.5)', background: '#fff', outline: 'none', maxWidth: '100%' }}>
                          ...
                          return mp.hasMeta ? (
                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11.5, fontWeight: 700, color: hecho ? '#2E6E6E' : '#A87A2C' }}>
                              <span style={{ width: 54, height: 5, borderRadius: 99, background: 'rgba(15,35,64,0.10)', overflow: 'hidden', display: 'inline-block' }}>
                                <span style={{ display: 'block', width: `${mp.pct * 100}%`, height: '100%', background: hecho ? '#2E6E6E' : (feat.color || ep.color) }} />
                              </span>
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ marginBottom: 16 }}>
                      ...
                      {investedMin > 0 && (
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'baseline', marginBottom: 9, padding: '7px 10px', borderRadius: 9, background: 'rgba(62,110,110,0.07)' }}>
                          <span style={{ fontSize: 12.5, fontWeight: 800, color: '#2E6E6E' }}>⏱ {fmtMin(investedMin)} invertidas en total</span>
                          <span style={{ fontSize: 11.5, color: 'rgba(20,35,61,0.5)' }}>· {daysWithTime} {daysWithTime === 1 ? 'día' : 'días'} con tiempo · ~{fmtMin(avgMin)}/día en promedio</span>
                        </div>
                      )}
                      {log.length === 0
    ...
    return (
      <div className="ep-md" style={{ display: 'flex', width: '100%', gap: 14, alignItems: 'stretch', minHeight: 420 }}>
        <div className="ep-md-list" style={{ flex: '0 0 320px', maxWidth: 360, maxHeight: 'calc(100dvh - 88px)', position: 'sticky', top: 12, alignSelf: 'flex-start', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 4, paddingRight: 4 }}>
          ...
          {/* Rutinas diarias como grupo ARRIBA de la lista (toggle 🔁 en la barra de arriba). */}
          {detShowRoutines && (() => {
            const rDay = mdDay || today
            const rMon = mondayISO(rDay)
            const rDi = (new Date(rDay + 'T00:00:00').getDay() + 6) % 7
            const routines = activeEpics.flatMap(e => (e.routines || []).map((r, ri) => ({ e, r, ri })))
            if (routines.length === 0) return <div style={{ fontSize: 11.5, color: 'rgba(20,35,61,0.45)', padding: '2px 4px 8px' }}>Sin rutinas diarias. Agrégalas en una épica.</div>
            const doneN = routines.filter(({ r }) => getRoutineWeek(r, rMon)[rDi]).length
            return (
              <div style={{ marginBottom: 6, paddingBottom: 8, borderBottom: '1px solid rgba(15,35,64,0.08)', display: 'flex', flexDirection: 'column', gap: 4 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 2px 2px', font: '700 9px/1 var(--font-ui)', letterSpacing: '.08em', textTransform: 'uppercase', color: '#5F5596' }}>🔁 Rutinas · {mdDay ? cap(new Date(rDay + 'T00:00:00').toLocaleDateString('es-MX', { weekday: 'short', day: 'numeric' }).replace('.', '')) : 'hoy'} <span style={{ color: 'rgba(95,85,150,0.7)', fontWeight: 800 }}>{doneN}/{routines.length}</span></div>
                {routines.map(({ e, r, ri }) => {
  ...
  return (
    <div style={{ minHeight: '100%' }}>
              ...
              {(featureOptions.length > 0 || hasSinFeature) && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: 9, marginBottom: 18 }}>
                  ...
                  {featureOptions.map(f => {
                    ...
                    const measurable = (f.kpis || []).find(k => milestoneProgress(k, featEpica).hasMeta)
                    const mp = measurable ? milestoneProgress(measurable, featEpica) : null
      ...
      {dayCloseOpen && (() => {
        ...
        // PLANEADO vs TRABAJADO: "planeado" = lo que AGENDASTE en el Planificador de /tiempo (bloques
        ...
        const plannedMin = cmpRows.reduce((s, r) => s + r.est, 0) + (showGenCmp ? genSchedPlan : 0)
        const realTaskMin = cmpRows.reduce((s, r) => s + r.real, 0) + (showGenCmp ? otherMinDay : 0)
        ...
        return (
          <>
          <div onClick={() => setDayCloseOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 78, background: 'rgba(10,22,42,0.5)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '20px 16px', overflow: 'auto' }}>
            <div role="dialog" aria-modal="true" aria-label="Cierre del día" onClick={e => e.stopPropagation()} className="ep-modal" style={{ width: '100%', maxWidth: 1180, background: '#fff', borderRadius: 18, boxShadow: '0 40px 80px -30px rgba(8,18,36,.7)', overflow: 'hidden' }}>
              <div style={{ height: 4, background: 'linear-gradient(90deg,#3E8E8E,#C2933A)' }} />
              <div style={{ padding: '18px 22px 22px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: 16 }}>
                  <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
                  {stat('✓', fDone.length, 'terminadas', '#2E6E6E')}
                  {stat('◐', fOpen.length, 'sin terminar', '#A87A2C')}
                  {stat('○', fUntouched.length, 'sin tocar', 'rgba(20,35,61,0.55)')}
                </div>
                {/* FILTRO POR ÉPICA: ver solo las tareas/rutinas de una épica ese día. */}
                {dayEpics.length > 1 && (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 12 }}>
                    <button onClick={() => setDcEpic('todas')} style={{ cursor: 'pointer', borderRadius: 99, padding: '5px 12px', font: '700 11.5px var(--font-ui)', border: dcEpic === 'todas' ? 'none' : '1px solid rgba(15,35,64,0.14)', background: dcEpic === 'todas' ? '#16365F' : '#fff', color: dcEpic === 'todas' ? '#fff' : 'rgba(20,35,61,0.6)' }}>Todas</button>
                    {dayEpics.map(e => (
                      <button key={e.id} onClick={() => setDcEpic(e.id)} style={{ cursor: 'pointer', borderRadius: 99, padding: '5px 12px', font: '700 11.5px var(--font-ui)', border: dcEpic === e.id ? 'none' : '1px solid rgba(15,35,64,0.14)', background: dcEpic === e.id ? e.color : '#fff', color: dcEpic === e.id ? '#fff' : 'rgba(20,35,61,0.6)', display: 'inline-flex', alignItems: 'center', gap: 6 }}><span style={{ width: 7, height: 7, borderRadius: 99, background: dcEpic === e.id ? '#fff' : e.color }} />{e.name}</button>
                    ))}
                  </div>
                )}
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, marginTop: 18, alignItems: 'flex-start' }}>
                  <div style={{ flex: '1 1 360px', minWidth: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
                    {fRoutines.length > 0 && (
                      <div>
                        {secLbl('Rutinas del día', <span style={{ fontSize: 10.5, fontWeight: 700, color: '#2E6E6E' }}>{fRoutines.filter(r => r.done).length}/{fRoutines.length} hechas</span>)}
                        {box(fRoutines.map(routineRow))}
                    ...
                    {generalRowsF.length > 0 && (
                      <div>
                        ...
                        {box(generalRowsF.map((o, k) => (
                          <div key={o.name + k} onClick={() => openOtherBlock(o.name)} title="Describir qué hiciste" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 11px', cursor: 'pointer', borderBottom: k < generalRowsF.length - 1 ? '1px solid rgba(15,35,64,0.05)' : 'none' }}>
                    ...
                    {fUntouched.length > 0 && (
                      <div>
                        {secLbl(refIsToday ? '○ No las tocaste hoy' : '○ Sin tocar', (
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            <button onClick={() => setDcSel(s => s.size >= fUntouched.length ? new Set() : new Set(fUntouched.map(x => planKey(x.e.id, x.t))))} style={{ cursor: 'pointer', border: 'none', background: 'transparent', font: '700 11px var(--font-ui)', color: 'rgba(20,35,61,0.5)' }}>{dcSel.size >= fUntouched.length && fUntouched.length ? 'ninguna' : 'todas'}</button>
                            {fUntouched.length > 8 && <button onClick={() => setDcShowAll(v => !v)} style={{ cursor: 'pointer', border: 'none', background: 'transparent', font: '700 11px var(--font-ui)', color: '#2E5A9E' }}>{dcShowAll ? 'ver menos ▴' : `ver todas (${fUntouched.length}) ▾`}</button>}
                          </div>
                        ))}
                        {dcSel.size > 0 && (
                        ...
                        {box((dcShowAll ? fUntouched : fUntouched.slice(0, 8)).map(selRow))}
                      </div>
                    )}
                  ...
                  <div style={{ flex: '1 1 280px', minWidth: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
                      ...
                      <div>
                        ...
                        <div style={{ borderRadius: 12, border: '1px solid rgba(15,35,64,0.09)', padding: '12px 14px', background: '#FBFAF6' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
                            ...
                            <div style={{ textAlign: 'right' }}><div className="serif" style={{ fontSize: 22, color: '#A87A2C', lineHeight: 1 }}>{hmm(plannedMin)}</div><div style={{ fontSize: 10, color: 'rgba(20,35,61,0.5)' }}>agendado</div></div>
                          </div>
                          ...
                          <div style={{ marginTop: 5, fontSize: 11, color: 'rgba(20,35,61,0.55)', fontWeight: 600 }}>{planPct}% de lo que agendaste{realTaskMin >= plannedMin ? ' · ✓ cumpliste' : ` · faltan ${hmm(Math.max(0, plannedMin - realTaskMin))}`}</div>
                          <div style={{ marginTop: 3, fontSize: 10, color: 'rgba(20,35,61,0.4)' }}>Agendado = los bloques que pusiste en el Planificador de /tiempo.</div>
                        </div>
                      </div>
                    )}
                    <div>
                    ...
                    {(() => { const sc = dayScores[refDay] ?? null; const scColor = sc == null ? '#94A3B8' : sc < 4 ? '#B0522E' : sc < 7 ? '#C2933A' : '#2E6E6E'; return (
                      <div>
                        {secLbl('⭐ Qué tan productivo fue', sc != null ? <button onClick={() => setDayScore(refDay, null)} style={{ cursor: 'pointer', border: 'none', background: 'transparent', font: '700 11px var(--font-ui)', color: 'rgba(20,35,61,0.45)' }}>quitar</button> : undefined)}
                        <div style={{ borderRadius: 12, border: '1px solid rgba(15,35,64,0.09)', padding: '12px 14px', background: '#FBFAF6' }}>
                          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 8 }}>
                            <span className="serif" style={{ fontSize: 34, lineHeight: .9, fontWeight: 600, color: scColor }}>{sc == null ? '—' : sc.toFixed(1)}</span>
                            <span style={{ fontSize: 13, color: 'rgba(20,35,61,0.5)', fontWeight: 600 }}>/ 10</span>
                          </div>
                          <input type="range" min={1} max={10} step={0.1} value={sc ?? 5} onChange={e => setDayScore(refDay, Number(e.target.value))} style={{ width: '100%', accentColor: scColor, cursor: 'pointer' }} />
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9.5, color: 'rgba(20,35,61,0.4)', marginTop: 2 }}><span>1</span><span>5</span><span>10</span></div>
                        </div>
                      </div>
                    ) })()}
                    <div>
                  ...
                  <div style={{ padding: '18px 20px 20px' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: 14 }}>
                      <div>
                        ...
                        <div style={{ marginTop: 6, fontSize: 12.5, color: 'rgba(20,35,61,0.6)' }}>Trabajaste <b style={{ color: '#2E6E6E' }}>{hmm(realTaskMin)}</b> de <b style={{ color: '#A87A2C' }}>{hmm(plannedMin)}</b> agendadas · <b>{planPct}%</b>{over > 0 ? ` · ${over} se pasaron` : ''}</div>
                        <div style={{ marginTop: 3, fontSize: 10.5, color: 'rgba(20,35,61,0.4)' }}>“Agendado” = los bloques que pusiste en el Planificador de /tiempo.</div>
                      </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {(() => {
                        const cmpRow = (key: string, name: string, color: string, est: number, real: number, done: boolean, onClick: (() => void) | null) => {
                          ...
                          const tone = real === 0 ? 'rgba(20,35,61,0.35)' : (est > 0 && real > est * 1.1) ? '#B0522E' : done ? '#2E6E6E' : '#C2933A'
                          ...
                          return (
                            <div key={key} onClick={onClick || undefined} title={onClick ? 'Ver la actividad' : undefined} style={{ cursor: onClick ? 'pointer' : 'default' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                                ...
                                <span style={{ flex: 1, minWidth: 0, fontSize: 13, color: '#16365F', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</span>
                        ...
                        return (<>
                          {cmpRows.map(r => cmpRow(r.t.id!, r.t.t, r.e.color, r.est, r.real, r.done, () => { setDcCompare(false); setDcPeek({ eId: r.e.id, tid: r.t.id! }) }))}
                          {showGenCmp && cmpRow('__gen', 'General / sin tarea', '#94A3B8', genSchedPlan, otherMinDay, true, null)}
                          {cmpRows.length === 0 && !showGenCmp && <div style={{ fontSize: 12.5, color: 'rgba(20,35,61,0.5)', textAlign: 'center', padding: '8px 0' }}>No agendaste nada este día en el Planificador.</div>}
                        </>)
                      })()}
                    </div>
                  </div>
